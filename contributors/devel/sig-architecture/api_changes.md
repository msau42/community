...
* You must add the new version to
  [pkg/apis/group_name/install/install.go](https://github.com/kubernetes/kubernetes/blob/v1.21.2/pkg/apis/apps/install/install.go).
* You must add the new version to
...
* You must setup storage for the new version in
  [pkg/registry/group_name/rest](https://github.com/kubernetes/kubernetes/blob/v1.21.2/pkg/registry/authentication/rest/storage_authentication.go).

You need to regenerate the generated code as instructed in the sections above.

Afterwards, sanity test the changes by bringing up a cluster (i.e.,
local-up-cluster.sh, kind, etc) and running `kubectl get
<resource>.<version>.<group>`.  [Integration tests](../sig-testing/integration-tests.md)
